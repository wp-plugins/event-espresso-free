...
<?php	
		...
		if (file_exists(EVENT_ESPRESSO_GATEWAY_DIR . "/mwarrior/settings.php")){
			require_once(EVENT_ESPRESSO_GATEWAY_DIR . "/mwarrior/settings.php");
			event_espresso_mwarrior_settings();
		}elseif (file_exists(EVENT_ESPRESSO_PLUGINFULLPATH."gateways/mwarrior/settings.php")){
			require_once(EVENT_ESPRESSO_PLUGINFULLPATH."gateways/mwarrior/settings.php");
			event_espresso_mwarrior_settings();
		}
		...
		global $espresso_premium; if ($espresso_premium != true)
				echo '<h2>'.__('Need more payment options?', 'event_espresso') . ' <a href="http://eventespresso.com/download/" target="_blank">'.__('Upgrade Now!', 'event_espresso').'</a></h2>';
?>
