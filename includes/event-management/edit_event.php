        ...
        <div class="submitbox" id="submitpost">
          <div id="minor-publishing">
            ...
            <div id="misc-publishing-actions">
              ...
              <div class="misc-pub-section <?php echo (function_exists('espresso_user_meta') && (espresso_member_data('role')=='espresso_event_admin' || espresso_member_data('role')=='espresso_group_admin' || current_user_can('administrator')) && $espresso_premium == true)?'':'misc-pub-section-last'; ?>" id="visibility2">
              	<a href="admin.php?page=events&amp;event_admin_reports=event_newsletter&amp;event_id=<?php echo $event_id ?>" title="<?php _e( 'Send Event Newsletter', 'event_espresso' ); ?>"><img src="<?php echo EVENT_ESPRESSO_PLUGINFULLURL ?>images/icons/email_go.png" width="16" height="16" alt="<?php _e( 'Newsletter', 'event_espresso' ); ?>" /></a> <a href="admin.php?page=events&amp;event_admin_reports=event_newsletter&amp;event_id=<?php echo $event_id ?>" title="<?php _e( 'Send Event Newsletter', 'event_espresso' ); ?>"><?php _e('Send Newsletter', 'event_espresso'); ?></a></div>
<?php 
if (function_exists('espresso_user_meta') && (espresso_member_data('role')=='espresso_event_admin' || espresso_member_data('role')=='espresso_group_admin' || current_user_can('administrator')) && $espresso_premium == true){
	...
	echo '<li><strong>'.__('Date Submitted:', 'event_espresso').'</strong> ' . event_date_display($event_meta['date_submitted'], get_option('date_format')) .'</li>';
	echo '</ul>';
	echo '</div>';
}?>
...
?>
      <div style="display: block;" id="event-meta" class="postbox">
		<div class="handlediv" title="Click to toggle"><br></div>
		<h3 class="hndle"><span><?php _e('Post to Facebook','event_espresso'); ?></span></h3>
		<div class="inside">
			<input type="checkbox" name="espresso_fb" id="espresso_fb" <?php echo ($fb_e_id ? "CHECKED=TRUE" : null);?>/> <?php _e('Post to Facebook', 'event_espresso'); ?>
<?php	if( !empty($fb_e_id) ){	?>
			<a href="http://www.facebook.com/event.php?eid=<?php echo  $fb_e_id; ?>" target="_blank"><?php _e('Event Page On Facebook', 'event_espresso'); ?></a>
<?php	}	?>
		</div>
	</div>     
<?php	}	?>
...
?>
    ...
    <div style="display: block;" id="event-questions" class="postbox">
      ...
      <h3 class="hndle"><span>
        <?php _e('Event Questions for Primary Attendee','event_espresso'); ?>
        </span></h3>
      <div class="inside">
        ...
        reset ($q_groups);
			...
			foreach ($q_groups as $question_group){
				...
				$group_name = $question_group->group_name;
					...
					echo '<p id="event-question-group-' . $group_id . '">
                                             <input value="' . $question_group_id . '" type="checkbox" ' . $visibility . ' name="add_attendee_question_groups[' . $question_group_id . ']" ' . $checked . ' /> <a href="admin.php?page=form_groups&amp;action=edit_group&amp;group_id=' . $question_group_id . '" title="edit" target="_blank">'. $group_name. "</a></p>";
			}
			if ($espresso_premium != true)
				echo __('Need more questions?', 'event_espresso') . ' <a href="http://eventespresso.com/download/" target="_blank">'.__('Upgrade Now!', 'event_espresso').'</a>';
            ?>
      ...
      <div style="display: block;" id="event-location" class="postbox">
        ...
        <div class="inside">
          <table width="100%" border="0" cellpadding="5">
            ...
            <?php if (function_exists( 'espresso_venue_dd' ) && $org_options['use_venue_manager'] == 'Y' && $espresso_premium == true){
						echo '<td><p><strong>'.__('Venue Information','event_espresso').'</strong></p>'.espresso_venue_dd($event_meta['venue_id']).'</td>';
					}else{?>
              <td>
...
<input size="20" tabindex="113"  type="text"  value="<?php echo $virtual_phone ?>" name="virtual_phone" />
    ...
    <?php  if ($espresso_premium == true){ ?>
        <div style="display: block;" id="event-meta" class="postbox">
            <div class="handlediv" title="Click to toggle"><br></div>
            <h3 class="hndle"><span><?php _e('Event Meta','event_espresso'); ?></span></h3>
            <div class="inside">
...
<input type="hidden" name="date_submitted" value="<?php echo $event_meta['date_submitted'];?>">
