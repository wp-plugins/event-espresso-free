...
if ( defined( 'EVENT_ESPRESSO_RECURRENCE_MODULE_ACTIVE' ) && $_POST['recurrence'] == 'Y' && count($recurrence_arr)== 0 && $_POST['recurrence_apply_changes_to'] >1){
...
else
{
    ...
    $overflow_event_id = ($_REQUEST['overflow_event_id'] == '') ? '0' : $_REQUEST['overflow_event_id'];
    $allow_overflow = ($_REQUEST['allow_overflow'] == '') ? 'N' : $_REQUEST['allow_overflow'];

    $additional_limit = $_REQUEST['additional_limit'];
    //$member_only=$_REQUEST['member_only'];
    $member_only = ($_REQUEST['member_only'] == '') ? 'N' : $_REQUEST['member_only'];
	...
	$registration_startT = $_REQUEST['registration_startT'];
	$registration_endT = $_REQUEST['registration_endT'];
	...
	$email_id = $_REQUEST['email_name'];


    $event_category = serialize( $_REQUEST['event_category'] );
    $event_discount = serialize( $_REQUEST['event_discount'] );
    ...
    $visible_on = array_key_exists('visible_on', $recurrence_arr)?$recurrence_arr['visible_on']:$_REQUEST['visible_on'];
    ...
    $item_groups = serialize( $_REQUEST['item_groups'] );
    
   	$event_mata['venue_id'] = $_REQUEST['venue_id'][0];
    ...
    $sql = array( 'event_name' => $event_name, 'event_desc' => $event_desc, 'display_desc' => $display_desc, 'display_reg_form'=>$display_reg_form, 'event_identifier' => $event_identifier,
    ...
    if ( $_REQUEST['start_time'] != '' )
    {
        foreach ( $_REQUEST['start_time'] as $k => $v ) {
            if ( $v != '' )
            {
				...
				$sql_times = "INSERT INTO " . EVENTS_START_END_TABLE . " (event_id, start_time, end_time, reg_limit) VALUES ('" . $event_id . "', '" . $v . "', '" . $_REQUEST['end_time'][$k] . "', ".$time_qty.")";
                //echo "$sql_times <br>";
    ...
    if ( $_REQUEST['event_cost'] != '' )
    {
        foreach ( $_REQUEST['event_cost'] as $k => $v ) {
            if ( $v != '' )
            {
				...
				$member_price_type = $_REQUEST['member_price_type'][$k] != '' ? $_REQUEST['member_price_type'][$k] : __('Members Admission', 'event_espresso');
				$member_price = $_REQUEST['member_price'][$k] ? $_REQUEST['member_price'][$k] : $v;
    ...
    switch ( $_REQUEST['create_post'] ){
		...
		if ($wpdb->num_rows >0 && $_REQUEST['delete_post'] == 'Y'){
