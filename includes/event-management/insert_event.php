...
else
{
		...
		$event_identifier=($_REQUEST['event_identifier'] == '') ? $event_identifier = sanitize_title_with_dashes($event_name.'-'.rand()) : $event_identifier = sanitize_title_with_dashes($_REQUEST['event_identifier']).rand();
		...
		//$end_date = array_key_exists('recurrence_start_date', $recurrence_arr)?$recurrence_arr['recurrence_start_date']:($_REQUEST['end_date']==''?$_REQUEST['recurrence_start_date']:$_REQUEST['end_date']);
                ...
                } elseif ($_REQUEST['visible_on'] != '') {
                    $visible_on = $_REQUEST['visible_on'];
                } elseif ($_REQUEST['visible_on'] == '' && count($recurrence_dates) > 0 ) {
		...
		$sql_data = array('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',
						  ...
						  '%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%d','%d','%d','%d','%d','%s','%s','%s');
		...
		if ($_REQUEST['start_time'] != ''){
			foreach ($_REQUEST['start_time'] as $k=>$v){
				if($v != '') {
					...
					$sql3="INSERT INTO ".EVENTS_START_END_TABLE." (event_id, start_time, end_time, reg_limit) VALUES ('".$last_event_id."', '".$v."', '".$_REQUEST['end_time'][$k]."', ".$time_qty.")";
					//echo "$sql3 <br>";
					if (!$wpdb->query($sql3)){
						$error = true;
					}
