            ...
            <div class="padding">
              <ul>
				...
				<li><strong><?php _e('Current Time', 'event_espresso'); ?>:</strong> 
                   <?php echo date(get_option('date_format'). ' ' .get_option('time_format')); ?> <a href="options-general.php" target="_blank"><br /><?php _e('Change timezone and date format settings?', 'event_espresso'); ?></a><br /><span class="red_text"><?php _e('Note:', 'event_espresso'); ?></span> <?php _e('You must set the time zone for your city, or the city closest to you. UTC time will not work.', 'event_espresso'); ?>
				</li>			
                <li>
            ...
            <div class="padding"> <a name="page_settings" id="page_settings"></a>
               <?php 
                ...
                <label for="event_ssl_active">
                  <?php _e('Use SSL for registration process?','event_espresso'); ?>
                </label>

                 <?php
                 //This needs to be cleaned up
                 $values=array(			
                     array('id'=>'','text'=> __('','event_espresso')),
                     array('id'=>'N','text'=> __('No','event_espresso')),
                    array('id'=>'Y','text'=> __('Yes','event_espresso'))
                    );   
                 echo select_input('event_ssl_active', $values, $org_options['event_ssl_active']);
                ...
                <font  size="-2" style="color:red">
                    (
                   <?php _e('If you plan on using one of the gateways that collects Credit Card info (e.g. PayPal PRO, Authorize.net AIM),
                       <br />you need to make sure a SSL is installed on this domain and Yes is selected in this dropdown.', 'event_espresso');
                       _e('<br />This feature is in beta.  Please notify support@eventespresso.com if you notice issues.', 'event-espresso');
                   ?>
                    )
                </font>
                </p>
              ...
              <p>
                ...
                <?php _e('shortcode. <br />This page can be hidden from navigation if desired, <br />but should always contain the [ESPRESSO_EVENTS] shortcode.', 'event_espresso'); ?>
                </font></p>
              <div id="registration_page_info" style="display:none">
                <h2><?php _e('Main Events Page', 'event_espresso'); ?></h2>
						<p><?php _e('This is the page that displays your events.', 'event_espresso'); ?></p>
						<p><?php _e('This page should contain the', 'event_espresso'); ?> <strong>[ESPRESSO_EVENTS]</strong> <?php _e('shortcode.', 'event_espresso'); ?></p>
              </div>
              <p>
...
but still viewable to the public (not password protected.)', 'event_espresso'); ?>
              ...
              <div id="return_url_info" style="display:none">
                <h2><?php _e('Auto Return URL', 'event_espresso'); ?></h2>
						<p><?php _e('The URL to which the payer"s browser is redirected after completing the payment; for example, a URL on your site that displays a "Thank you for your payment" page.', 'event_espresso'); ?></p>
						<p><?php _e('This page should contain the <strong>[ESPRESSO_PAYMENTS]</strong> shortcode.', 'event_espresso'); ?></p>
						<p class="red_text"><strong><?php _e('ATTENTION:', 'event_espresso'); ?></strong><br /><?php _e('This page should be hidden from from your navigation menu. Exclude pages by using the "Exclude Pages" plugin from http://wordpress.org/extend/plugins/exclude-pages/ or using the "exclude" parameter in your "wp_list_pages" template tag. Please refer to http://codex.wordpress.org/Template_Tags/wp_list_pages for more inforamation about excluding pages.', 'event_espresso'); ?></p>
              </div>
              <p>
              ...
              <div id="cancel_return_info" style="display:none"><h2><?php _e('Cancel Return URL', 'event_espresso'); ?></h2>
               <p><?php _e('A URL to which the payer\'s browser is redirected if payment is cancelled; for example, a URL on your website that displays a "Payment Canceled" page.', 'event_espresso'); ?></p>
						<p><?php _e('This should be a page on your website that contains a cancelled message. No short tags are needed.', 'event_espresso'); ?></p>
						<p class="red_text"><strong><?php _e('ATTENTION:', 'event_espresso'); ?></strong><br /><?php _e('This page should be hidden from from your navigation menu. Exclude pages by using the "Exclude Pages" plugin from http://wordpress.org/extend/plugins/exclude-pages/ or using the "exclude" parameter in your "wp_list_pages" template tag. Please refer to http://codex.wordpress.org/Template_Tags/wp_list_pages for more inforamation about excluding pages.', 'event_espresso'); ?></p>
              </div>
              <p>
                ...
                <?php _e('shortcode. <br />This page should hidden from your navigation, <br />but still viewable to the public (not password protected.)', 'event_espresso'); ?>
                </font></p>
              <div id="notify_url_info" style="display:none"><h2><?php _e('Notify URL', 'event_espresso'); ?></h2>
               
						<p><?php _e('The URL to which PayPal posts information about the transaction, in the form of Instant Payment Notification messages.', 'event_espresso'); ?></p>
						<p><?php _e('This page should contain the', 'event_espresso'); ?> <strong>[ESPRESSO_TXN_PAGE]</strong> <?php _e('shortcode.', 'event_espresso'); ?></p>
						<p class="red_text"><strong><?php _e('ATTENTION:', 'event_espresso'); ?></strong><br /><?php _e('This page should be hidden from from your navigation menu. Exclude pages by using the "Exclude Pages" plugin from http://wordpress.org/extend/plugins/exclude-pages/ or using the "exclude" parameter in your "wp_list_pages" template tag. Please refer to http://codex.wordpress.org/Template_Tags/wp_list_pages for more inforamation about excluding pages.', 'event_espresso'); ?></p>
              </div>
               <p>
...
?>
	...
	echo select_input('email_before_payment', $values, $org_options['email_before_payment']);?>
              ...
              <p>
                ...
                <input name="payment_subject" size="50" type="text" value="<?php echo stripslashes_deep($org_options['payment_subject']);?>" />
              </p>
              <div class="postbox">
                <textarea class="theEditor" id="payment_message" name="payment_message"><?php echo stripslashes_deep($org_options['payment_message']);?></textarea>
              ...
              <div class="postbox">
                <textarea class="theEditor" id="success_message" name="success_message"><?php echo stripslashes_deep($org_options['message']);?></textarea>
        ...
        <input class="button-primary" type="submit" name="Submit" value="<?php _e('Save Options', 'event_espresso'); ?>" id="save_organization_saetting_5" />
          ...
          <li><h2><?php _e('Advanced Features', 'event_espresso'); ?></h2><hr /></li>
<?php 
...
?>
		<li><div class="metabox-holder">
			<div class="postbox">
  			<h3>
              <?php _e('Optional Event Settings','event_espresso'); ?>
            </h3><div class="padding"><p><?php echo __('Please purchase a', 'event_espresso')  ?> <a href="http://eventespresso.com/download/" target="_blank"><?php echo __('support license', 'event_espresso') ?></a> <?php echo __('to gain access to these features.', 'event_espresso') ?></p>
            <p><?php _e('Additional features include:', 'event_espresso'); ?></p>
            <ol>
            <li><?php _e('Upcoming events widget in the admin dashboard', 'event_espresso'); ?></li>
            <li><?php _e('Registration limits on time slots', 'event_espresso'); ?></li>
            <li><?php _e('Ability to display short descriptions in the event listings', 'event_espresso'); ?></li>
            <li><?php _e('Custom post types for events', 'event_espresso'); ?></li>
            <li><?php _e('Attendee pre-approval feature', 'event_espresso'); ?></li>
            <li><?php _e('Event Venue/Staff Manager', 'event_espresso'); ?></li>
            </ol>
            </div>
            </div>
            </div>
            ...
            </h3><div class="padding"><p><?php echo __('Please purchase a', 'event_espresso')  ?> <a href="http://eventespresso.com/download/" target="_blank"><?php echo __('support license', 'event_espresso') ?></a> <?php echo __('to gain access to this feature.', 'event_espresso') ?></p>
